...
export async function POST(req: NextRequest) {
  ...
  const query = await supabase.from(SupabaseTable.PARTNERSHIP_REQUEST).insert([
    {
      [PartnershipRequestColumn.NAME]: body.name,
      [PartnershipRequestColumn.CONTACT]: body.contact,
      [PartnershipRequestColumn.BRAND_NAME]: body.brandName,
      [PartnershipRequestColumn.REQUEST_STATE]: StoreState.WAIT,
      [PartnershipRequestColumn.ADDITIONAL]: body.additional,
    },
  ]);
